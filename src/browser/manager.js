...
export class BrowserManager {
  ...
  async launch(headless = this.headless) {
    ...
    this.launching = (async () => { try { const { chromium } = await import('patchright'); await mkdir(this.profile, { recursive: true }); this.context = await chromium.launchPersistentContext(this.profile, { channel: this.channel, headless, locale: 'en-AU', timezoneId: 'Australia/Brisbane', viewport: { width: 1440, height: 900 } }); this.context.on('close', () => { this.context = undefined; }); return this.context; }
  ...
  async fetchPage(url, { provider, debugDir, settleMs = 3000 } = {}) {
    ...
    try { await page.goto(url, { waitUntil: 'domcontentloaded', timeout: this.timeout }); await page.waitForTimeout(settleMs); const [title, html] = await Promise.all([page.title(), page.content()]); if (blocked(status, title, html)) throw new ProviderBlockedError(`${provider} returned a bot-protection page (HTTP ${status ?? '?'}, ${html.length} bytes). Run "property-search setup" headed.`); if (debugDir) { await mkdir(debugDir, { recursive: true }); await Promise.all([writeFile(`${debugDir}/page.html`, html), writeFile(`${debugDir}/network.json`, JSON.stringify(payloads, null, 2)), page.screenshot({ path: `${debugDir}/page.png`, fullPage: true })]); } return { html, title, status, url: page.url() }; }
    catch (error) { if (error.code) throw error; if (/timeout/i.test(error.message)) throw new ProviderTimeoutError(`${provider} timed out loading ${url}`); throw error; }
