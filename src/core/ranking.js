...
export function rank(properties, c) {
  return properties.map((p) => {
    ...
    if (c.minLandAreaM2 && (p.landAreaM2 ?? 0) >= c.minLandAreaM2) { score += w.landMinimum; reasons.push(`${p.landAreaM2}m² land`); if (p.landAreaM2 >= c.minLandAreaM2 * 1.2) score += w.landSignificantlyExceeds; }
